...
$this->refs = 'https://prestahero.com/';
    ...
    {
        $tabs= ['AdminEtsHB', 'AdminEtsHBBase'];
        foreach ($tabs as $tab) {
            $tabObj = Tab::getInstanceFromClassName($tab);
            if($tabObj->id)
            {
                $tabObj->delete();
            }
    ...
    public function getContent()
    {
        ...
        if(Tools::isSubmit('submitBulkDelete') && ($ids = array_map('intval',Tools::getValue('bulk_action_selected_html_boxs'))))
        {
            ...
            {
                $this->context->cookie->__set('_success', $this->l('Deleted box selected succesfully')) ;
            }
        ...
        if(Tools::isSubmit('submitBulkEnable') && ($ids = array_map('intval',Tools::getValue('bulk_action_selected_html_boxs'))))
        {
            ...
            {
                $this->context->cookie->__set('_success', $this->l('Enabled box selected succesfully')) ;
                $this->context->cookie->write();
            }
            Tools::redirectAdmin($this->getAdminModuleLink());
            $this->context->cookie->write();
        ...
        if(Tools::isSubmit('submitBulkDisable') && ($ids = array_map('intval',Tools::getValue('bulk_action_selected_html_boxs'))))
        {
            ...
            {
                $this->context->cookie->__set('_success', $this->l('Disabled box selected succesfully')) ;
                $this->context->cookie->write();
            }
            Tools::redirectAdmin($this->getAdminModuleLink());
            $this->context->cookie->write();
        ...
        if(Tools::isSubmit('change_enabled') && ($id_ets_hb_html_box = (int)Tools::getValue('id_ets_hb_html_box')))
        {
            ...
            if($htmlBox->update())
            {
                $this->context->cookie->__set('_success', $this->l('Updated succesfully')) ;
                $this->context->cookie->write();
                Tools::redirectAdmin($this->getAdminModuleLink());
            }
        }
        if(Tools::isSubmit('del') && ($id_ets_hb_html_box = (int)Tools::getValue('id_ets_hb_html_box')))
        {
            ...
            if($htmlBox->delete())
            {
                $this->context->cookie->__set('_success', $this->l('Deleted successfull')) ;
                $this->context->cookie->write();
        ...
        {
            $this->_html .= $this->displayConfirmation($this->context->cookie->__get('_success'));
            $this->context->cookie->__unset('_success');
            $this->context->cookie->write();
        }
        if(Tools::isSubmit('addNewBox') || (Tools::isSubmit('edithtml_box') && Tools::getValue('id_ets_hb_html_box')) )
        {
    ...
    public function saveHTMLBox()
    {
        ...
        if($this->_errors)
        {
            ...
            {
                if($htmlBox->update())
                    $this->context->cookie->__set('_success', $this->l('Edited successfull'));
                else
                ...
                if($htmlBox->add())
                {
                    $this->context->cookie->__set('_success', $this->l('Added successfull')) ;
                }
                else
    ...
    public function getFieldsValues($htmlBox)
    {
        ...
        if (!is_array($htmlBox->html)) {
            $htmlBox->html = array();
        }

        foreach (Language::getLanguages(false) as $lang) {
            $fields["html"][$lang["id_lang"]] = Tools::getValue("html_".$lang["id_lang"], isset($htmlBox->html[$lang["id_lang"]]) ? $htmlBox->html[$lang["id_lang"]] : '');
        }
        return $fields;
    }
    public function displayHooks($hook_name)
    {
        $hooks = HBHtmlbox::getHTMLBoxByHook($hook_name, $this->context->language->id);
    ...
    public function getPositions($value)
    {
        ...
        return $this->display(__FILE__,'positions.tpl');
    }
